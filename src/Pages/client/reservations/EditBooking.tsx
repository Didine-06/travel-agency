...
export default function EditBooking() {
    ...
    const load = async () => {
      try {
        ...
        setForm({
          ...
          travelDate: bookingData.travelDate ? new Date(bookingData.travelDate).toISOString().slice(0, 10) : '',
        });
      } catch (err) {
        if (cancelled) return;
        const message = err instanceof Error ? err.message : 'Erreur lors du chargement de la réservation';
        setError(message);
        setBooking(null);
      } finally {
  ...
  return (
    <>
      ...
      <div
        className={`fixed top-0 right-0 h-full w-[500px] max-w-[100vw] bg-white dark:bg-gray-900 border-l border-gray-200 dark:border-gray-700 z-50 flex flex-col transform transition-transform duration-200 ease-out ${open ? 'translate-x-0' : 'translate-x-full'}`}
        role="dialog"
        aria-modal="true"
      >
        <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Modifier réservation</h2>
            <p className="text-xs text-gray-500 dark:text-gray-400 break-all">{bookingId}</p>
          </div>
          <button
            onClick={() => {
              if (busy) return;
            ...
            className="px-3 py-2 text-sm font-medium border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors text-gray-900 dark:text-gray-100"
          >
            Fermer
          </button>
        </div>
            ...
            <>
              {booking && (
                <div className="rounded-lg border border-gray-200 dark:border-gray-700 p-3 bg-gray-50 dark:bg-gray-800/40">
                  <div className="text-sm font-medium text-gray-900 dark:text-gray-100">{booking.package.title}</div>
                  <div className="text-xs text-gray-500 dark:text-gray-400">
                    {booking.package.destination.city}, {booking.package.destination.country}
                  </div>
                  <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">Statut: {booking.status}</div>
                </div>
              )}

              <div className="grid grid-cols-1 gap-4">
                <label className="block">
                  <span className="block text-sm font-medium text-gray-700 dark:text-gray-300">Date de voyage</span>
                  <input
                    type="date"
                ...
                <div className="grid grid-cols-2 gap-3">
                  <label className="block">
                    <span className="block text-sm font-medium text-gray-700 dark:text-gray-300">Adultes</span>
                    <input
                      type="number"
                      min={1}
                      value={form.numberOfAdults}
                      onChange={(e) => setForm({ ...form, numberOfAdults: Number(e.target.value) })}
                      className="mt-1 w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </label>

                  <label className="block">
                    <span className="block text-sm font-medium text-gray-700 dark:text-gray-300">Enfants</span>
                    <input
                      type="number"
                      min={0}
                ...
                <label className="block">
                  <span className="block text-sm font-medium text-gray-700 dark:text-gray-300">Prix total</span>
                  <input
                    type="number"
                    min={0}
