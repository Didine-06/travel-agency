...
function ClientReservation() {
  ...
  const deleteOne = async (id: string) => {
    ...
    if (response.isSuccess) {
      const message =
        response?.message || t("reservations.messages.deleteSuccess");
      ...
      const errorMessage =
        response?.message || t("reservations.messages.deleteError");
  ...
  const deleteMany = async (ids: string[]) => {
    ...
    if (response.isSuccess) {
      const message =
        response?.message || t("reservations.messages.deleteManySuccess");
      toast.success(message);
      setSelectedBookings(new Set());
      setConfirmDeleteOpen(false);
      setDeleteTargetIds([]);
      ...
      const errorMessage =
        response?.message || t("reservations.messages.deleteManyError");
      toast.error(errorMessage);
      setConfirmDeleteOpen(false);
      setDeleteTargetIds([]);
    }
  ...
  return (
    <div className="flex flex-col h-full relative">
                        ...
                        currentBookings.map((booking) => (
                          ...
                          >
                            <td className="px-4 py-4">
                              <input
                                type="checkbox"
                                checked={selectedBookings.has(
                                  String(booking.id)
                                )}
                                onChange={() =>
                                  handleSelectBooking(String(booking.id))
                                }
                                className="w-4 h-4 text-blue-600 bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 rounded focus:ring-blue-500 focus:ring-2"
                              />
                            </td>
                            <td className="px-4 py-4">
                              ...
                              <div className="text-xs text-gray-500 dark:text-gray-400">
                                {t("reservations.duration", {
                                  count: booking.package.duration,
                                })}
                              </div>
                            </td>
                            <td className="px-4 py-4">
                            ...
                            <td className="px-4 py-4">
                              <div className="text-sm text-gray-900 dark:text-gray-100">
                                {t("reservations.travelers.adult", {
                                  count: booking.numberOfAdults,
                                })}
                              </div>
                              {booking.numberOfChildren > 0 && (
                                <div className="text-xs text-gray-500 dark:text-gray-400">
                                  {t("reservations.travelers.child", {
                                    count: booking.numberOfChildren,
                                  })}
                                </div>
                              )}
                            </td>
                            ...
                            <td className="px-4 py-4 text-right">
                              <div className="flex justify-end items-center">
                                <button
                                  onClick={(e) =>
                                    toggleDropdown(String(booking.id), e)
                                  }
                      ...
                      <div className="relative">
                        ...
                        {openDropdown === String(booking.id) && (
                          <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 z-10">
                            ...
                            <button
                              onClick={() =>
                                requestDeleteOne(String(booking.id))
                              }
        ...
        <div className="fixed bottom-16 md:bottom-0 left-0 right-0 md:left-auto md:right-auto md:relative bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 px-4 py-3 shadow-lg md:shadow-sm z-10 flex-shrink-0">
          <div className="flex items-center justify-between max-w-full">
            <div className="text-sm text-gray-700 dark:text-gray-300">
              ...
              {t("reservations.pagination.perPage")}
              <span className="hidden md:inline ml-2 text-gray-500 dark:text-gray-400">
                ({filteredBookings.length}{" "}
                {t(
                  `reservations.pagination.${
                    filteredBookings.length > 1 ? "results_plural" : "results"
                  }`
                )}
                )
              </span>
      ...
      <ConfirmDeleteModal
        ...
        entityLabel={t(
          deleteTargetIds.length > 1 ? "entities.bookings" : "entities.booking"
        )}
      ...
      <ReservationContext.Provider
        value={{ reloadBookings: () => loadBookings(true) }}
      >
        <Outlet />
      </ReservationContext.Provider>
    </div>
